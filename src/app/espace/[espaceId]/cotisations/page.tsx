...
import { cotisationStats, getCotisations, getEspace } from "@/lib/data";
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        action={
          <Button asChild>
            <Link href={`/espace/${espaceId}/cotisations/nouvelle`}>
      ...
      {cotisations.length === 0 ? (
        <EmptyState
          ...
          action={
            <Button asChild>
              <Link href={`/espace/${espaceId}/cotisations/nouvelle`}>Créer une cotisation</Link>
            </Button>
          }
        />
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex items-center justify-between border-t border-ledger-line pt-3 text-xs text-muted-foreground">
                  <span>{formatFCFA(c.montant)} / membre · {c.paiements.length} membres</span>
