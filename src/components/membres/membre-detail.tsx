...
import { MembreQRCode } from "@/components/membres/membre-qr-code";
import { useCotisations, useMembre } from "@/lib/selecteurs";
import { formatDate, formatFCFA } from "@/lib/format";

export function MembreDetail({ espaceId, membreId }: { espaceId: string; membreId: string }) {
  const membre = useMembre(espaceId, membreId);
  const cotisationsEspace = useCotisations(espaceId);

  if (!membre) notFound();

  const cotisations = cotisationsEspace
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Card className="ledger-card sm:col-span-2">
          <CardHeader>
            <CardTitle className="text-[15px] font-medium">Carte membre</CardTitle>
          </CardHeader>
          <CardContent>
            <MembreQRCode espaceId={espaceId} membreId={membreId} nomComplet={`${membre.prenom} ${membre.nom}`} />
          </CardContent>
        </Card>
      </div>
    </>
